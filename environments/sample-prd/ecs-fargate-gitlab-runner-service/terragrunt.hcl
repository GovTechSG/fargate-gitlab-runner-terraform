...
inputs = merge(local.vars.inputs,
  {
    ...
    managers_configs = {
      ...
      prd_tool2_tool4 : {
        tags : ["prd", "tool4"]
        limit : 5 # Check the available IPs in worker subnet
        worker_docker_image : "${local.vars.inputs.aws_account_id}.dkr.ecr.${local.vars.inputs.aws_region}.amazonaws.com/<WORKER_DOCKER_IMAGE_3>"
        worker_cpu : 512
        worker_memory : 2048
        worker_ecs_cluster_arn : "arn:aws:ecs:${local.vars.inputs.aws_region}:${local.vars.inputs.aws_account_id}:cluster/<ECS_CLUSTER_NAME>"
        worker_aws_region : local.vars.inputs.aws_region
        worker_subnet_id : "WORKER_SUBNET_3"
        worker_security_group_id : "WORKER_SECURITY_GROUP_3"
        worker_ssh_user : "user_2"
        worker_task_role_arn : "arn:aws:iam::${local.vars.inputs.aws_account_id}:role/<WORKER_IAM_ROLE_3>"
      }
    }
